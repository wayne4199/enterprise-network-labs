...
The objective is to understand how Layer 2, First-Hop Redundancy, Routed Link, Static Routing, Internet Simulation, Linux host configuration, and CML configuration preservation issues appear, how to identify them, and how to recover from them.
...
* CML Extract Configs limitations
* Lab export/import recovery procedures
...
# Scenario 20 — CML Extract Configs Saves Only Some Nodes

## Symptoms

After selecting all booted nodes and clicking CML `EXTRACT CONFIGS`, only some nodes appear to have saved configurations.

Cisco device configurations are preserved, but Linux-based node configurations may not be preserved.

Affected node types may include:
...
Check whether Cisco network devices retain their configurations:
...
Check whether Linux-based nodes still have expected OS-level settings:
...
CML `EXTRACT CONFIGS` is primarily intended for Cisco network device configuration extraction.

It reliably applies to:
...
It should not be treated as a full filesystem backup for Linux-based nodes.

Linux OS-level files and runtime state are outside the normal Cisco startup-config extraction model.

---

## Recovery

Use different preservation methods by node type.
...
Restore Netplan configuration from `configs.md`.
...
### Tiny Core Linux Node

Restore `/opt/bootlocal.sh` from `configs.md` and run:
...
Reapply temporary IP/Gateway settings only when verification is required.
...
# Scenario 21 — Linux Node Configuration Missing After CML Export / Import

## Symptoms

After exporting, downloading, or re-importing the CML lab, some Linux-based node settings may be missing or inconsistent.

Examples:

* SERVER1 loses Netplan static IP/Gateway
* MGMT-SRV1 loses Netplan static IP/Gateway
* INTERNET-SRV loses Tiny Core startup configuration
* Linux Desktop endpoints lose temporary IP/Gateway
...
```text
SERVER1    = 10.10.70.10/24, default via 10.10.70.254
MGMT-SRV1  = 10.10.40.10/24, default via 10.10.40.254
```
...
```text
INTERNET-SRV = 203.0.113.10/24, default via 203.0.113.1
```
...
CML lab export/import should not be assumed to preserve all Linux OS-level configuration in the same way Cisco startup-config is preserved.

Linux-based node configuration must be documented and verified separately.

---

## Recovery

Restore Linux node settings from `configs.md`.
...
Reapply temporary IP/Gateway only when testing is required.
...
Confirm SERVER1, MGMT-SRV1, and INTERNET-SRV are reachable before continuing to later phases.

---

# Scenario 22 — DNS Resolver Not Tested in PHASE 01
...
* CML Extract Configs limitations
* CML export/import recovery strategy
